...
.. php:class:: SimpleWorkflow

    A simple workflow

    .. php:attr:: enableRollback
    ...
    .. php:method:: run(Deployment $deployment)
        ...
        Sequentially execute the stages for each node, so first all nodes will go
        through the initialize stage and
        then the next stage will be executed until the final stage is reached and
        the workflow is finished.

        A rollback will be done for all nodes as long as the stage switch was not
        completed.
    ...
    .. php:method:: setEnableRollback($enableRollback)

        :type $enableRollback: bool
        :param $enableRollback:
        :returns: \TYPO3\Surf\Domain\Model\SimpleWorkflow

    .. php:method:: isEnableRollback()

        :returns: bool

    .. php:method:: getStages()
