...
final class ScpTask extends Task implements ShellCommandServiceAwareInterface
{
    ...
    public function execute(Node $node, Application $application, Deployment $deployment, array $options = [])
    {
        ...
        $destinationArgument = $node->isLocalhost() ? $remoteTransferPath : sprintf('%s@%s:%s', $node->getUsername(), $node->getHostname(), $remoteTransferPath);
        ...
        $this->shell->executeOrSimulate(sprintf('cd %1$s/; tar %3$s -zcf %2$s -C %1$s .', $localPackagePath, $fileName, $this->getExcludes($options)), $localhost, $deployment, false, false);

        // Transfer tarball to target server
        $this->shell->executeOrSimulate(sprintf('scp %s/%s %s', $localPackagePath, $fileName, $destinationArgument), $localhost, $deployment);

        // Extract tarball on server in release path
        $this->shell->executeOrSimulate(sprintf('tar -zxvf %s/%s -C %s', $remoteTransferPath, $fileName, $releasePath), $node, $deployment);

        // Delete tarball on localhost and Server
        $this->shell->executeOrSimulate(sprintf('rm -rf %s/%s', $remoteTransferPath, $fileName), $node, $deployment);
        $this->shell->executeOrSimulate(sprintf('rm -rf %s/%s', $localPackagePath, $fileName), $localhost, $deployment);
    }
     ...
     * @return string
     */
    private function getExcludes(array $options)
    {
        $excludes = ['.git'];
