 ...
 */
class DumpAutoloadTask extends \TYPO3\Surf\Domain\Model\Task {
	...
	/**
	 * Build the composer command to "dump-autoload --no-dev" in the given $path.
	 *
	...
	protected function buildComposerInstallCommand($manifestPath, array $options) {
		...
		if (empty($options['composerArguments'])) {
			$options['composerArguments'] = '';
		}
		return sprintf('cd %s && %s dump-autoload ' . $options['composerArguments'] . ' --no-ansi --no-interaction --no-dev', escapeshellarg($manifestPath), escapeshellcmd($options['composerCommandPath']));
	}
